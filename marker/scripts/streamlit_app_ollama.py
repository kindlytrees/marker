...
def translate_with_ollama(text: str, target_language: str = "中文") -> str:
    """使用 ollama 将文本翻译为目标语言（非流式版本）"""
    prompt = f"""请将以下 Markdown 文本翻译为{target_language}，保持 Markdown 格式不变（即保留所有的符号、链接、代码块等）：请直接输出翻译后的内容，不要添加任何解释。\n\n原文：{text}"""
    ...
    try:
        ...
        return f"翻译错误: {str(e)}"
def translate_with_ollama_stream(text: str, target_language: str = "中文") -> str:
    """使用 ollama 将文本翻译为目标语言"""
    #ollama_url = "http://localhost:11434/api/generate"
    
    prompt = f"""{text}，请将前面的文字翻译为{target_language}，保持 Markdown 格式不变（即保留所有的符号、链接、代码块等）：请直接输出翻译后的内容，不要添加任何解释。"""
    ...
    if output_format == "markdown":
        ...
        if do_translate:
            with col2_bottom:
                ...
                try:
                    # 获取流式翻译结果
                    stream_generator = translate_with_ollama_stream(display_original)
